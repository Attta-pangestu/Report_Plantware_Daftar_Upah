#!/usr/bin/env python3
"""
Daftar Upah HTML Template Engine - Fixed Version
Optimized for proper column fitting and clean layout
"""
...
class DaftarUpahTemplateEngineFixed:
    """
    Fixed HTML Template Engine for Daftar Upah (Payroll Reports)

    Improvements:
    - Better column organization (56 columns -> 47 columns)
    - Fixed width table (2900px) for consistent layout
    ...
    def render_employee_row(self, index, employee):
        """Render a single employee row with optimized layout"""
    ...
    def generate_report(self, template_file="daftar_upah_template_fixed.html",
                       data_file="../large_data.json",
                       output_file=None):
        """Generate complete HTML payroll report with fixed layout"""
        try:
            ...
            print(f"Table Width: 2900px (Fixed)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate Daftar Upah HTML Report (Fixed Layout)')
    parser.add_argument('--template', '-t', default='daftar_upah_template_fixed.html',
                        help='Template file name (default: daftar_upah_template_fixed.html)')
